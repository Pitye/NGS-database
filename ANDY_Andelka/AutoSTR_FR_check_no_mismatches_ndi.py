...
def autoSTR_FR_check_no_mismatches(in_directory, out_directory, xml_directory, CSV_CE_directory, reports_directory, no_reads_for_validation, CheckIfSampleInDatabase, dbPass):
            ...
            for marker in markers:
                ...
                if marker in list(Data_CE[sample_NGS].keys()):
                    ...
                    if genotype_CE == genotype_NGS:
                        ...
                        #print (sample_NGS, marker, 'genotype_CE is ', genotype_CE, 'genotype_NGS is', genotype_NGS  )
                        f.writelines(["\n" + sample_NGS + "," + marker + "," + genotype_CE[0]+ "," + genotype_CE[1] + "," + genotype_NGS[0] + "," + genotype_NGS[1]])
            
            Auto_STR_Head[sample_NGS]['No_mismatches']= no_mismatches
            #print(sample_NGS, 'no_mismatches', no_mismatches, "\n") 
            f.writelines(["\n" + sample_NGS + "," + "no_mismatches" + "," + str(no_mismatches) +"\n"])
        else:
            #print(sample_NGS, ' is not in xml or csv CE files', "\n")
            f.writelines(["\n" + sample_NGS + "," + " is not in xml or csv CE files" + "\n"])
            Auto_STR_Head[sample_NGS]['No_mismatches']= no_mismatches
            for marker in markers:
                if int(Auto_STR_Data[sample_NGS][marker][0][3]) >= no_reads_for_validation and int(Auto_STR_Data[sample_NGS][marker][1][3]) >= no_reads_for_validation:
                    Auto_STR_Data[sample_NGS][marker][0] = Auto_STR_Data[sample_NGS][marker][0] + ['validated_no_reads']
    ...
    if CheckIfSampleInDatabase:
        ...
        for sample_name in sample_names:
            ...
            if row_count == 0:
                #print (sample_name, 'is not in database')
                f.writelines(["\n" + sample_name + "," + " is not in database"]) 
            else:
                #print (sample_name, 'entry already exists in database')
