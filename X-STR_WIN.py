...
csv_list_2 = os.listdir(out_directory + os.path.normpath('/') + sheets[2] +  os.path.normpath('/'))
#
#
...
for file in csv_list_2:
    ...
    for row_X in csv.reader(open(out_directory +  os.path.normpath('/') + sheets[2] +  os.path.normpath('/') + file), delimiter=','):
...
for sample_name_X in sample_names_X:
    ...
    db.commit()
    print(sample_name_X, " inserted in database")
