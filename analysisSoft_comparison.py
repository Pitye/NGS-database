...
for sample in GM_samples:
    #create empty raw_dictionary and dictionary with homo or hetero alleles for markers
    sample_dict = {marker : [] for marker in PowerSeq_markers}
    sorted_sample_dict = {marker : [] for marker in PowerSeq_markers}
    
    #select true readings to sample_dict according to marker's heterozygozity 
    for marker in PowerSeq_markers:
        sorted_markers = sorted(GM_Auto_STR_Data_raw[sample][marker], key = lambda x: int(x[3]),reverse = True)
        ...
        if len (sorted_markers) > 1:
            if int(sorted_markers[1][3])/int(sorted_markers[0][3]) > float(PowerSeq_markers_heteroyzgozity_dict[marker]):
                sample_dict[marker].append(sorted_markers[0])
        ...
        if sample_dict['Amelogenin'][0][1] == 'chrY':
    ...
    GM_Auto_STR_Data[sample] = sorted_sample_dict
    
print(GM_Auto_STR_Data)
